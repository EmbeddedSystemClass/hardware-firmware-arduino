  ...
  public:
    ...
    byte editStr(byte x, byte y, PGM_P pMask, char buffer[], char length, byte key) {      
      ...
      if (bInvalidatePos) {
        ...
        Display.drawLine(x + pos * TEXTWIDTH * 2, y, x + (pos + 1) * TEXTWIDTH * 2 - 2, y, YELLOW);
        Display.drawLine(x + pos * TEXTWIDTH * 2, y3, x + (pos + 1) * TEXTWIDTH * 2 - 2, y3, YELLOW);
        
        bInvalidatePos = false;
      }
      
      if (bInvalidateText) {
        Display.displayText(x, y2, 2, buffer, YELLOW, BACKCOLOR);
  ...
  public:
    ...
    void setOptions(byte x, byte y, char* options[], byte count, byte width, byte input) {
      ...
      if (bInvalidateSelection) {
        ...
        Display.drawRect(x, y + selected * (TEXTHEIGHT * 2 + 2), width + 8, TEXTHEIGHT * 2 + 4, YELLOW);
        bInvalidateSelection = false;
      }
      if (bInvalidateText) {
        y += 3;
        for (int i=0; i < count; i++) {
            Display.displayText(x + 4, y + i * (TEXTHEIGHT * 2 + 2), 2, options[i], YELLOW, BACKCOLOR);
        }
        bInvalidateText = false;
      }
    }
